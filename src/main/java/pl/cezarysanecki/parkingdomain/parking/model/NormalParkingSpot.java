...
public class NormalParkingSpot implements ParkingSpot {
    ...
    public NormalParkingSpot(ParkingSpotId parkingSpotId, int capacity) {
        this(parkingSpotId, capacity, Set.of());
    }

    public NormalParkingSpot(
            ParkingSpotId parkingSpotId,
            int capacity,
            Set<Vehicle> parkedVehicles) {
    ...
    public Either<ParkingFailed, VehicleParkedEvents> park(Vehicle vehicle) {
        if (isNotEnoughSpaceFor(vehicle)) {
            return announceFailure(new ParkingFailed(parkingSpotId, vehicle.getVehicleId()));
        }
