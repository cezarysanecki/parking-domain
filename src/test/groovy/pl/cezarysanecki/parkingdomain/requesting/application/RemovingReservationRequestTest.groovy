...
class RemovingReservationRequestTest extends Specification {
  ...
  def "should cancel reservation request"() {
    ...
    and:
      reservationRequestRepository.getBy(reservationRequest.requestId) >> reservationRequest
    
    when:
      def result = cancellingReservationRequest.cancelRequest(reservationRequest.requestId)
    
    then:
      result.isSuccess()
      result.get().with {
        assert it == reservationRequest.requestId
      }
    and:
